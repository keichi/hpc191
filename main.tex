...
図\ref{fig:node}にSX-Aurora TSUBASA C401-8ノードの構成を示す．
SX-Aurora TSUBASAはVector Host (VH) とVector Engine (VE) の2種のプロセッサが搭載した
ヘテロジニアスな計算機である．VEはPCI Expressカード上に実装されたベクトルプロセッサであり，
アプリケーションの実行を担う．VEは2,048ビット幅のベクトルレジスタを活用したベクトル演算命令を備え，
高帯域幅メモリとの密結合されており，特にメモリ律速のアプリケーションにおいて高い性能を発揮する．
...
有するため，SX-Aurora TSUBASA C401-8 1ノードあたり計41.78\,TFLOP/sの演算性能を有する．
504ノードのシステム全体では21.05\,PFLOP/sの演算性能を有する．
メモリ帯域については同様にノードあたり19.8\,TB/s，システム全体では9.97\,PB/sである．
2023年8月現在稼働している国内のシステムと比較すると，ピーク演算性能においてはWisteria/BDEC-01に次いで
4位，メモリ帯域幅においては富岳に次いで2位の性能である．
...
2\,MBのL3キャッシュが新設されている．L3キャッシュはソフトウェアによってバイパスすることが可能であり，
選択的キャッシングによって
局所性の高いデータと低いデータが混在するアプリケーションにおいてキャッシュを有効活用できる．
...
2.54\,TB/sへ1.6倍に向上している．さらに，メモリ容量は48\,GBから96\,GBへ2倍に拡大され，LLC容量は
16\,MBから64\,MBへ4倍に拡大している．LLCの帯域幅は3.0\,TB/sから6.4\,TB/sに2.13倍に向上している．

これらの改良に加え，リストベクトル演算を加速するための専用ハードウェア新設や，
単精度演算におけるアラインメント制約の緩和など，アーキテクチャに様々な改良が加えられている．
これらの改良点の詳細な性能分析については文献~\cite{Takahashi2023}を参照されたい．

\subsection{相互結合網とストレージ}

AOBA-Sは計40基のサーバラックから構成される．
計算ノードについては，1ラックに16 VHが搭載されており，32ラックに計504 VHが収容されている．
これらに加え，フロントエンドノードおよび管理・監視サーバ群に2ラック，ネットワーク機器が2ラック，
...
ストレージにはDN社製LustreアプライアンスであるES400NVX2を2基採用している．
...
提供している．NLCはBLAS，LAPACK，SBLAS，ScaLAPACK，FFTW互換のインターフェースを有しており，
既存ソフトウェア資産の移植を容易にしている．
...
キャッシュラインサイズが256\,Bと2倍の大きさであるため，STREAMのような粒度の大きい，連続メモリアクセスにおいてGPUが有利である可能性がある．
...
JUWELS Booster上での性能測定結果~\cite{Brunst2021}を参照する．
...
図\ref{fig:spechpc-l}に各システムにおけるLargeスイートの性能を示す．
全体の問題サイズは$4096^3$に増大しているものの，1ランクに配布される部分格子のサイズは$2^5$のまま
であり，ベクトル長の短さが性能劣化の要因となっている．
LBM，CloverLeaf，miniWeatherでは特にプロセス数数が増加するとBoosterとの性能差が拡大しており，
Boosterに比べスケーラビリティが劣っている．この原因として，(1) 通信性能の差 と (2) プログラム起動時間の差
が考えられるため，検証を試みた．
...
\begin{figure}[tb]
  ...
  \caption{SPEChpc Largeスイートの実行時間内訳}\label{fig:spechpc-profile1}
\end{figure}
...
検討するほか，他システムにおいて大規模な実行を行い，Launch Overhead Timeの分析を実施する．
...
本稿では東北大学サイバーサイエンスセンターにおいて2023年8月より稼働開始したAOBA-Sの設計を示し，
...
今後はさらに性能評価および性能分析を進め，得られた知見を利用者に対して還元していく．
