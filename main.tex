...
ベンチマークの実効においては\verb|-i 1000 -x 500|オプションを指定し，
...
\begin{figure}[tb]
  ...
  \end{minipage}
  \caption{SPEChpc Largeサイズの性能}\label{fig:spechpc-l}
\end{figure}

SPEChpcの公式ウェブサイト\footnote{\url{https://www.spec.org/hpc2021/results/}}において公開されている
Largeサイズの計測結果は限られているため，ここでは文献~\cite{Brunst2022}において計測が
なされた，米Texas Advanced Computing CenterのFronteraおよび独J\"{u}lich Supercomputing Centreの
JUWELS Booster上での性能測定結果と比較する．FronteraはノードあたりIntel Xeon Platinum 8280を2
...
図\ref{fig:spechpc-l}は

図\ref{fig:spechpc-profile}にNEC MPI\footnote{\url{https://sxauroratsubasa.sakura.ne.jp/documents/mpi/pdfs/g2am01-NEC_MPI_User_Guide_ja.pdf}}
のプロファイラ機能を用いて計測した，各ベンチマークの実行時間に占めるMPI通信時間を示す．
